...
for t in range(M):
    ...
    schedule[A].append((B, t))
    schedule[B].append((A, t))

search_que = PriorityQueue()
search_que.put((0, 1))
while search_que.qsize() > 0:
    next = search_que.get()
    if node[next[1]] != -1 and node[next[1]] > next[0]:
        continue
    next_node = next[1]
    prev_time = next[0]
    for s in schedule[next_node]:
        n = (prev_time - s[1] + M - 1) // M
        arrive = s[1] + M * n + 1
        if node[s[0]] == -1 or node[s[0]] > arrive:
            node[s[0]] = arrive
            search_que.put((arrive, s[0]))
